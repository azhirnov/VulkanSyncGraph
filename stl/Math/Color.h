...
{
	template <typename T>
	struct hash< VSA::RGBAColor<T> >
	{
		ND_ size_t  operator () (const VSA::RGBAColor<T> &value) const
		{
			return	size_t(	VSA::HashOf( value.r ) + VSA::HashOf( value.g ) +
							VSA::HashOf( value.b ) + VSA::HashOf( value.a ));
		}
	};
	

	template <>
	struct hash< VSA::DepthStencil >
	{
		ND_ size_t  operator () (const VSA::DepthStencil &value) const
		{
			return size_t(VSA::HashOf( value.depth ) + VSA::HashOf( value.stencil ));
		}
	};
